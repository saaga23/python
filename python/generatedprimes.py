#geratedprimes.py
#contains the defination of the is_prime function
from math import sqrt

def is_prime(n):
    """ Returns Truennonnegative integer n is prime;
...
def prime_sequence(begin, end):
    """ Generates the squence of prime numbers between begin and end. """
    for value in range(begin, end + 1):
        if is_prime(value):   #See if value is prime
            yield value    # produce the prime value
            
            
def main():
    """ Experiments with the prime number generator"""
    min_value = int(input("Enter starts of range: "))
